table 50082 "SSD Posted Quality Order Hdr"
{
    Caption = 'Posted Quality Order Header';
    DrillDownPageID = "Posted Quality Order List";
    LookupPageID = "Posted Quality Order List";
    ...
    fields
    {
        ...
        {
            CalcFormula = sum("SSD Posted Quality Order Hdr"."Lot Size" where("Template Type"=filter(Routing|RcvdCoil), "Entry Source Type"=field("Entry Source Type"), "Source Document No."=field("Source Document No."), "Order No."=field("No.")));
            Editable = false;
            FieldClass = FlowField;
        ...
        field(50111; "No of Archived Version"; Integer)
        {
            CalcFormula = max("SSD Posted Quality Ord Hdr Ar"."Version No." where("No."=field("No.")));
            Description = 'QLT[A]';
            FieldClass = FlowField;
        ...
        field(50122; "Minimum Value"; Decimal)
        {
            AutoFormatType = 2;
            BlankZero = true;
            DecimalPlaces = 2: 2;
        ...
        field(50123; "Maximum Value"; Decimal)
        {
            AutoFormatType = 2;
            DataClassification = CustomerContent;
    ...
    begin
        //<<< ALLE[5.51]
        if "Inspection Value2" < "Minimum Value" then if Confirm('Average Value is less than Min Value.Do you want to continue', false, bol2)then exit;
        if "Maximum Value" <> 0 then begin
            if "Inspection Value2" > "Maximum Value" then if Confirm('Average Value is more than Max Value than.Do you want to continue', false, bol2)then exit;
        end;
    //>>> ALLE[5.51]
    end;
