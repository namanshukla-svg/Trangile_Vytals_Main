page 50361 "SSD Service Confimation List"
{
    ApplicationArea = Basic, Suite;
    Caption = 'Service Confirmation';
    ...
    SourceTableView = where("Document Type"=const(Order), Status=const(Released), "SSD Order Type"=filter("SSD Purchase Order Type"::Service), "Confirmation Pending"=filter(true));
    UsageCategory = Lists;
    AdditionalSearchTerms = 'SPO Confirmation List';
    ...
    actions
    {
        area(processing)
        {
            group("P&osting")
            {
                ...
                {
                    ApplicationArea = Suite;
                    Caption = 'P&ost Confimation Receipt';
                    Ellipsis = true;
                    Image = PostOrder;
                    ShortCutKey = 'F9';
                    ...
                    trigger OnAction()
                    begin
                        SSDPurchaseSubscribers.UpdateVendorConfimation(Rec);
                    end;
                }
            }
        }
