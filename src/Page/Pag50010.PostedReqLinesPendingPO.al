Page 50010 "Posted Req. Lines Pending PO"
{
    ...
    begin
        ...
        PurchOrderLine.SetRange("Document Type", RecPurchHeader."Document Type");
        PurchOrderLine.SetRange("Document No.", RecPurchHeader."No.");
        PurchOrderLine.SetRange(Type, RecReqLine.Type);
        PurchOrderLine.SetRange("No.", RecReqLine."No.");
        if not PurchOrderLine.Find('-')then begin
            PurchOrderLine.Init;
            ...
            PurchOrderLine.Validate(Type, RecReqLine.Type);
        ...
        begin
            if PurchOrderLine."Quantity Received" > 0 then Error(Text007, PurchOrderLine."Document No.", PurchOrderLine."Line No.", PurchOrderLine."No.");
        end;
        PurchOrderLine.Validate(Quantity, PurchOrderLine.Quantity + RecReqLine."Requested PO Qty");
        if RecPurchHeader."Prices Including VAT" then PurchOrderLine.Validate("Direct Unit Cost", RecReqLine."Direct Unit Cost" * (1 + PurchOrderLine."VAT %" / 100))
        else
        ...
        if(RecReqLine."Indent No." <> '') and (RecReqLine."Indent Line No." <> 0)then begin
            ...
            if PostedIndentLineLocal.Find('-')then repeat case RecReqLine."Created PO Doc. type" of RecReqLine."created po doc. type"::Order: begin
        ...
        if PurchasingCode.Get(RecReqLine."Purchasing Code")then if PurchasingCode."Special Order" then begin
        ...
        TempJnlLineDim.SETRANGE("Journal Line No.",RecReqLine."Line No.");
        IF TempJnlLineDim.FIND('-') THEN BEGIN
          REPEAT
            IF DocDim.GET(DATABASE::"Purchase Line",
    ...
    begin
        ...
        if ProdOrderComponent.Find('-')then repeat SubOrderComponents.Init;
        ...
        ProdOrderComponent.SetFilter("Prod. Order No.", RecPurchLine."Prod. Order No.");
        ProdOrderComponent.SetRange(ProdOrderComponent."Prod. Order Line No.", RecPurchLine."Prod. Order Line No.");
        if ProdOrderComponent.Find('-')then repeat SubOrderCompListVend.Init;
    ...
    begin
        ...
        if(PurchLine."Quantity (Base)" - PurchLine."Reserved Qty. (Base)") > ReqLine."Demand Quantity (Base)" then ReservQty:=ReqLine."Demand Quantity (Base)"
        else
            ReservQty:=PurchLine."Quantity (Base)" - PurchLine."Reserved Qty. (Base)";
    //SSDU ReservMgt.SetPurchLine(PurchLine);
    /*
        CASE ReqLine."Demand Type" OF
          DATABASE::"Prod. Order Component":
           ...
           {   ReserveProdOrderComp.CreateReservation(
            ...
            //CML-Check Ashish 150108 Finish
    ...
    begin
        ...
        if RequisitionLineLocal.Find('-')then repeat RequisitionLineLocal.TestField("Pending PO", true);
