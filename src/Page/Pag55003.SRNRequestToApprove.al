page 55003 "SRN-Request To Approve" //SRN-IG-20250903-0002
{
    ApplicationArea = Basic, Suite;
    Caption = 'SRN-Request To Approve';
    CardPageID = "SSD Service Receipt";
    DataCaptionFields = "Buy-from Vendor No.";
    Editable = false;
    PageType = List;
    RefreshOnActivate = true;
    SourceTable = "Purchase Header";
    SourceTableView = where("Document Type" = const(Order), Status = const(Released), "SSD Order Type" = filter(<> "SSD Purchase Order Type"::Inventory), "SRN Approval Status" = filter('Pending For Approval'));
    UsageCategory = Lists;
    AdditionalSearchTerms = 'SPO Receipt List, Service Receipt Overview';
        ...
        area(Processing)
        {
            group("A&pproval")
            {
                ...
                action(Approve)
                {
                    ...
                    Image = Approve;
                    ShortCutKey = 'F8';
                    ToolTip = 'Approve this document by setting SRN Approver Status to Released.';

                    trigger OnAction()
                    begin
                        Rec.TestField("SRN Approval Status", Rec."SRN Approval Status"::"Pending For Approval");
                        if not Confirm('Do you want to approve document %1?', false, Rec."No.") then
                            exit;
                        Rec."SRN Approval Status" := Rec."SRN Approval Status"::Released;
                        Rec.Modify();
                        Message('Document %1 approved and status set to Released.', Rec."No.");
                    end;
                }
            }
        }
    ...
    begin
        ...
        DimFilter := '';
        UserSetup.Reset(); //IG_DS
        UserSetup.SetRange("SSD SRN Approver", UserId);
        UserSetup.SetFilter("Indent Costcen Code", '<>%1', '');
        if UserSetup.FindFirst() then
            repeat
                if DimFilter <> '' then
                    DimFilter += '|';
                DimFilter += UserSetup."Indent Costcen Code";
            until UserSetup.Next() = 0;
        if DimFilter <> '' then begin
            Rec.FilterGroup(2);
            Rec.SetFilter("Shortcut Dimension 1 Code", DimFilter);
            Rec.FilterGroup(0);
        end else begin
            Rec.FilterGroup(2);
            Rec.SetRange("Shortcut Dimension 1 Code", DimFilter);
            Rec.FilterGroup(0);
        end;
    end;
