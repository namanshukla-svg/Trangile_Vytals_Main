Page 50162 "Arch P. Rcpt. Q.Order Subform"
{
    Caption = 'Arch P. Rcpt. Q.Order Subform';
    DeleteAllowed = false;
    Editable = false;
    InsertAllowed = false;
    ModifyAllowed = false;
    ...
    SourceTableView = sorting("Document No.", "Version No.", "Line No.")order(ascending)where("Template Type"=const(Receipt));
    ApplicationArea = All;
        ...
        area(processing)
        {
            group("&Order")
            {
                ...
                action("Line Comments")
                {
                    ...
                    trigger OnAction()
                    begin
                        //This functionality was copied from page #50161. Unsupported part was commented. Please check it.
