Report 50010 "RTGS Document"
{
    // ALLE 2.15....NEFT/RTGS
    DefaultLayout = RDLC;
    RDLCLayout = './Layouts/RTGS Document.rdl';
    ApplicationArea = all;
    UsageCategory = Lists;
    ...
    dataset
    {
        ...
        {
            DataItemTableView = sorting("Document No.", "Posting Date", Amount)where("Transfer Type"=filter(RTGS), "Vendor Bank Account"=filter(<>''), "NEFT / RTGS Code"=filter(<>''));
            ...
            column(Document_No_______Document_No__;'Document No.:  ' + "Document No.")
            {
            }
            column(CompInfo_Name_Control1000000020; CompInfo.Name)
            {
            }
            column(RespCent_Address______RespCent__Address_2__RespCent_City_____RespCent__Post_Code_; RespCent.Address + '  ' + RespCent."Address 2" + RespCent.City + ' ' + RespCent."Post Code")
            {
            }
