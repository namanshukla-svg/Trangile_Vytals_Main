report 50378 "GSTR-1 File Format New"
{
    Caption = 'GSTR-1 File Format New';
    ProcessingOnly = true;
    UseRequestPage = true;
    ...
    dataset
    {
        dataitem(Integer; Integer)
        {
            ...
            begin
                case NatureofSupply of NatureofSupply::" ": ERRor(FileFormatErr);
                NatureofSupply::B2B: begin
                    MakeExcelBodyB2B();
                    CreateandOpenExcel(B2BTxt);
                end;
                NatureofSupply::B2CL: begin
                    MakeExcelBodyB2CL();
                    CreateandOpenExcel(B2CLTxt);
                end;
                NatureofSupply::B2CS: begin
                    MakeExcelBodyB2CS();
                    CreateandOpenExcel(B2CSTxt);
                end;
                NatureofSupply::AT: begin
                    MakeExcelBodyAT();
                    CreateandOpenExcel(ATTxt);
                end;
                NatureofSupply::ATADJ: begin
                    MakeExcelBodyATADJ();
                    CreateandOpenExcel(ATADJTxt);
                end;
                NatureofSupply::CDNR: begin
                    MakeExcelBodyCDNR();
                    CreateandOpenExcel(CDNRTxt);
                end;
                NatureofSupply::CDNUR: begin
                    MakeExcelBodyCDNUR();
                    CreateandOpenExcel(CDNURTxt);
                end;
                NatureofSupply::EXP: begin
                    MakeExcelBodyEXP();
                    CreateandOpenExcel(EXPTxt);
                end;
                NatureofSupply::HSN: begin
                    MakeExcelBodyHSN();
            ...
            area(content)
            {
                group(Options)
                {
                    ...
                    field(FileFormat; NatureofSupply)
                    {
                        ...
                        trigger OnValidate()
                        begin
                            case NatureofSupply of NatureofSupply::B2CL: B2CLimit:=250000;
                            NatureofSupply::B2CS: B2CLimit:=250000;
                            else
                                B2CLimit:=0.00 end;
                        end;
                    }
    ...
    INVNO: Label 'Invoice Number';
    CustomerNameTxt: Label 'Customer Name';
    CustomerNoTxt: Label 'Customer No.';
    NatureofSupply: Option " ", B2B, B2CL, B2CS, EXP, CDNUR, CDNR, ATADJ, AT, HSN;
    ...
    begin
        ...
        TempExcelBuffer.AddColumn(CustomerNoTxt, false, '', true, false, false, '', TempExcelBuffer."Cell Type"::Text);
        TempExcelBuffer.AddColumn(CustomerNameTxt, false, '', true, false, false, '', TempExcelBuffer."Cell Type"::Text);
    ...
    begin
        ...
        GSTR1B2BQuery.Open();
        while GSTR1B2BQuery.Read()do FillExcelBufferB2B(GSTR1B2BQuery);
    end;
    ...
    begin
        TempExcelBuffer.NewRow();
        if GSTR1B2BQuery.Reverse_Charge then TempExcelBuffer.AddColumn(GSTR1B2BQuery.Location__Reg__No_, false, '', false, false, false, '', TempExcelBuffer."Cell Type"::Text)
        else
        ...
        begin
            ...
            if GSTR1B2BQuery.Finance_Charge_Memo then TempExcelBuffer.AddColumn(GetInvoiceValueFinCharge(GSTR1B2BQuery.Document_No_), false, '', false, false, false, '0.00', TempExcelBuffer."Cell Type"::Number)
            else
        ...
        // .........
        if GSTR1B2BQuery.Source_No_ <> '' then TempExcelBuffer.AddColumn(GSTR1B2BQuery.Source_No_, false, '', false, false, false, '', TempExcelBuffer."Cell Type"::Text)
        else
        ...
        if GSTR1B2BQuery.Source_No_ <> '' then TempExcelBuffer.AddColumn(GSTR1B2BQuery.Source_No_, false, '', false, false, false, '', TempExcelBuffer."Cell Type"::Text)
            ...
            TempExcelBuffer.AddColumn('', false, '', false, false, false, '', TempExcelBuffer."Cell Type"::Text);
        // .........
        if GSTR1B2BQuery.Buyer_Seller_State_Code <> '' then TempExcelBuffer.AddColumn(GSTR1B2BQuery.State_Code__GST_Reg__No__ + '-' + GSTR1B2BQuery.Description, false, '', false, false, false, '', TempExcelBuffer."Cell Type"::Text)
        else
            TempExcelBuffer.AddColumn('', false, '', false, false, false, '', TempExcelBuffer."Cell Type"::Text);
        if GSTR1B2BQuery.Reverse_Charge then TempExcelBuffer.AddColumn(YLbl, false, '', false, false, false, '', TempExcelBuffer."Cell Type"::Text)
        else
        ...
        if GSTR1B2BQuery.e_Comm__Operator_GST_Reg__No_ <> '' then TempExcelBuffer.AddColumn(GSTR1B2BQuery.e_Comm__Operator_GST_Reg__No_, false, '', false, false, false, '', TempExcelBuffer."Cell Type"::Text)
        else
            TempExcelBuffer.AddColumn('', false, '', false, false, false, '', TempExcelBuffer."Cell Type"::Text);
        ...
        GSTR1B2BGSTPer.Open();
        while GSTR1B2BGSTPer.Read()do if GSTR1B2BGSTPer.GST_Jurisdiction_Type = GSTR1B2BGSTPer.GST_Jurisdiction_Type::Intrastate then TempExcelBuffer.AddColumn(2 * GSTR1B2BGSTPer.GST__, false, '', false, false, false, '', TempExcelBuffer."Cell Type"::Number)
            else
                TempExcelBuffer.AddColumn(GSTR1B2BGSTPer.GST__, false, '', false, false, false, '', TempExcelBuffer."Cell Type"::Number);
        if GSTR1B2BQuery.GST_Jurisdiction_Type = GSTR1B2BQuery.GST_Jurisdiction_Type::Intrastate then TempExcelBuffer.AddColumn(Abs(GSTR1B2BQuery.GST_Base_Amount / 2), false, '', false, false, false, '0.00', TempExcelBuffer."Cell Type"::Number)
        else
        ...
        if GSTR1B2BCessAmt.Read()then TempExcelBuffer.AddColumn(Abs(GSTR1B2BCessAmt.GST_Amount), false, '', false, false, false, '0.00', TempExcelBuffer."Cell Type"::Number)
        else
    ...
    begin
        ...
        GSTR1B2CLQuery.Open();
        while GSTR1B2CLQuery.Read()do if GetInvoiceValue(GSTR1B2CLQuery.Document_No_, "GST Document Type"::Invoice) >= B2CLimit then FillExcelBufferForB2CL(GSTR1B2CLQuery);
    end;
    ...
    begin
        ...
        if GSTR1B2CLQuery.Buyer_Seller_State_Code <> '' then TempExcelBuffer.AddColumn(GSTR1B2CLQuery.State_Code__GST_Reg__No__ + '-' + GSTR1B2CLQuery.Description, false, '', false, false, false, '', TempExcelBuffer."Cell Type"::Text)
        else
            TempExcelBuffer.AddColumn('', false, '', false, false, false, '', TempExcelBuffer."Cell Type"::Text);
        ...
        GSTR1B2CLPer.Open();
        while GSTR1B2CLPer.Read()do TempExcelBuffer.AddColumn(GSTR1B2CLPer.GST__, false, '', false, false, false, '0.00', TempExcelBuffer."Cell Type"::Number);
        ...
        if GSTR1B2CLCessAmt.Read()then TempExcelBuffer.AddColumn(Abs(GSTR1B2CLCessAmt.GST_Amount), false, '', false, false, false, '0.00', TempExcelBuffer."Cell Type"::Number)
        else
            TempExcelBuffer.AddColumn(0.00, false, '', false, false, false, '0.00', TempExcelBuffer."Cell Type"::Number);
        if GSTR1B2CLQuery.e_Comm__Operator_GST_Reg__No_ <> '' then TempExcelBuffer.AddColumn(GSTR1B2CLQuery.e_Comm__Operator_GST_Reg__No_, false, '', false, false, false, '', TempExcelBuffer."Cell Type"::Text)
        else
            TempExcelBuffer.AddColumn('', false, '', false, false, false, '', TempExcelBuffer."Cell Type"::Text);
    end;
    ...
    begin
        TempExcelBuffer.NewRow();
        TempExcelBuffer.AddColumn(GSTINUINTxt, false, '', true, false, false, '', TempExcelBuffer."Cell Type"::Text);
        TempExcelBuffer.AddColumn(INVNO, false, '', true, false, false, '', TempExcelBuffer."Cell Type"::Text);
        TempExcelBuffer.AddColumn(InvoiceDateTxt, false, '', true, false, false, '', TempExcelBuffer."Cell Type"::Text);
        TempExcelBuffer.AddColumn(CustomerNoTxt, false, '', true, false, false, '', TempExcelBuffer."Cell Type"::Text);
        TempExcelBuffer.AddColumn(CustomerNameTxt, false, '', true, false, false, '', TempExcelBuffer."Cell Type"::Text);
        ...
        TempExcelBuffer.AddColumn(RateTxt, false, '', true, false, false, '', TempExcelBuffer."Cell Type"::Text);
        TempExcelBuffer.AddColumn(TaxableValueTxt, false, '', true, false, false, '', TempExcelBuffer."Cell Type"::Text);
        TempExcelBuffer.AddColumn(TaxAmount, false, '', true, false, false, '', TempExcelBuffer."Cell Type"::Text);
    ...
    var
        GSTR1B2CSQuery: Query GSTR1B2CSQueryNEW;
    begin
        MakeExcelHeaderB2CS();
        ...
        GSTR1B2CSQuery.Open();
        while GSTR1B2CSQuery.Read()do FillExcelBufferForB2CS(GSTR1B2CSQuery);
    end;
    local procedure FillExcelBufferForB2CS(GSTR1B2CSQuery: Query GSTR1B2CSQueryNEW)
    var
        State: Record State;
    ...
    begin
        TempExcelBuffer.NewRow();
        // GURMEET
        if GSTR1B2CSQuery.Reverse_Charge then TempExcelBuffer.AddColumn(GSTR1B2CSQuery.Location__Reg__No_, false, '', false, false, false, '', TempExcelBuffer."Cell Type"::Text)
        else
            TempExcelBuffer.AddColumn(GSTR1B2CSQuery.Buyer_Seller_Reg__No_, false, '', false, false, false, '', TempExcelBuffer."Cell Type"::Text);
        if GSTR1B2CSQuery.Document_No_ <> '' then TempExcelBuffer.AddColumn(GSTR1B2CSQuery.Document_No_, false, '', false, false, false, '', TempExcelBuffer."Cell Type"::Text)
        else
            TempExcelBuffer.AddColumn('', false, '', false, false, false, '', TempExcelBuffer."Cell Type"::Text);
        if GSTR1B2CSQuery.Document_No_ <> '' then TempExcelBuffer.AddColumn(GSTR1B2CSQuery.Posting_Date, false, '', false, false, false, '', TempExcelBuffer."Cell Type"::Date)
        else
            TempExcelBuffer.AddColumn('', false, '', false, false, false, '', TempExcelBuffer."Cell Type"::Text);
        if GSTR1B2CSQuery.Source_No_ <> '' then TempExcelBuffer.AddColumn(GSTR1B2CSQuery.Source_No_, false, '', false, false, false, '', TempExcelBuffer."Cell Type"::Text)
        else
            TempExcelBuffer.AddColumn('', false, '', false, false, false, '', TempExcelBuffer."Cell Type"::Text);
        if GSTR1B2CSQuery.Source_No_ <> '' then TempExcelBuffer.AddColumn(GSTR1B2CSQuery.Name, false, '', false, false, false, '', TempExcelBuffer."Cell Type"::Text)
        else
            TempExcelBuffer.AddColumn('', false, '', false, false, false, '', TempExcelBuffer."Cell Type"::Text);
        // GURMEET
        if GSTR1B2CSQuery.e_Comm__Operator_GST_Reg__No_ <> '' then TempExcelBuffer.AddColumn(ELbl, false, '', false, false, false, '', TempExcelBuffer."Cell Type"::Text)
        else
            TempExcelBuffer.AddColumn(UpperCase(OtherECommTxt), false, '', false, false, false, '', TempExcelBuffer."Cell Type"::Text);
        if State.Get(GSTR1B2CSQuery.Buyer_Seller_State_Code)then TempExcelBuffer.AddColumn(State."State Code (GST Reg. No.)" + '-' + State.Description, false, '', false, false, false, '', TempExcelBuffer."Cell Type"::Text)
        else
            TempExcelBuffer.AddColumn('', false, '', false, false, false, '', TempExcelBuffer."Cell Type"::Text);
        ...
        GSTR1B2CSPer.Open();
        while GSTR1B2CSPer.Read()do if GSTR1B2CSPer.GST_Jurisdiction_Type = GSTR1B2CSPer.GST_Jurisdiction_Type::Intrastate then TempExcelBuffer.AddColumn(2 * GSTR1B2CSPer.GST__, false, '', false, false, false, '', TempExcelBuffer."Cell Type"::Number)
            else
        ...
        while GSTR1B2CSIntraAmt.Read()do GSTRB2CSIntraAmount:=GSTR1B2CSIntraAmt.GST_Base_Amount;
        ...
        GSTR1B2CSInter.Open();
        while GSTR1B2CSInter.Read()do GSTR1B2CSInterBaseAmt:=GSTR1B2CSInter.GST_Base_Amount;
        ...
        // gurmeet
        TempExcelBuffer.AddColumn(GSTR1B2CSQuery.GST_Amount, false, '', false, false, false, '', TempExcelBuffer."Cell Type"::Number);
        TempExcelBuffer.AddColumn(GSTR1B2CSQuery.Quantity, false, '', false, false, false, '', TempExcelBuffer."Cell Type"::Number);
        // gurmeet
        ...
        if GSTR1B2CSCessAmt.Read()then begin
            ...
            while GSTR1B2CSIntraCess.Read()do GSTR1IntraCess:=GSTR1B2CSIntraCess.GST_Amount;
            ...
            while GSTR1B2CInterCess.Read()do GSTR1InterCess:=GSTR1B2CInterCess.GST_Amount;
        end;
        ...
        if GSTR1B2CSQuery.e_Comm__Operator_GST_Reg__No_ <> '' then TempExcelBuffer.AddColumn(GSTR1B2CSQuery.e_Comm__Operator_GST_Reg__No_, false, '', false, false, false, '', TempExcelBuffer."Cell Type"::Text)
        else
            TempExcelBuffer.AddColumn('', false, '', false, false, false, '', TempExcelBuffer."Cell Type"::Text);
    end;
    ...
    begin
        ...
        GSTR1ATQuery.Open();
        while GSTR1ATQuery.Read()do FillExcelBufferForAT(GSTR1ATQuery);
    end;
    ...
    begin
        ...
        GSTR1HSNQuery.Open();
        while GSTR1HSNQuery.Read()do FillExcelBufferForHSN(GSTR1HSNQuery);
    end;
    ...
    begin
        ...
        GSTR1HSNGSTAmt.Open();
        while GSTR1HSNGSTAmt.Read()do HSNCGSTAmt:=GSTR1HSNGSTAmt.GST_Amount;
        GSTR1HSNGSTAmt.SetRange(GST_Component_Code, SGSTLbl);
        GSTR1HSNGSTAmt.Open();
        while GSTR1HSNGSTAmt.Read()do HSNSGSTAmt:=GSTR1HSNGSTAmt.GST_Amount;
        GSTR1HSNGSTAmt.SetRange(GST_Component_Code, IGSTLbl);
        GSTR1HSNGSTAmt.Open();
        while GSTR1HSNGSTAmt.Read()do HSNIGSTAmt:=GSTR1HSNGSTAmt.GST_Amount;
        GSTR1HSNGSTAmt.SetRange(GST_Component_Code, CessLbl);
        GSTR1HSNGSTAmt.Open();
        while GSTR1HSNGSTAmt.Read()do HSNCessAmt:=GSTR1HSNGSTAmt.GST_Amount;
        ...
        GSTR1HSNQty.Open();
        while GSTR1HSNQty.Read()do if GSTR1HSNQty.GST_Jurisdiction_Type = GSTR1HSNQty.GST_Jurisdiction_Type::Intrastate then begin
        ...
        if GSTR1HSNQuery.HSN_SAC_Code <> '' then TempExcelBuffer.AddColumn(GSTR1HSNQuery.Description, false, '', false, false, false, '', TempExcelBuffer."Cell Type"::Text)
        else
            TempExcelBuffer.AddColumn('', false, '', false, false, false, '', TempExcelBuffer."Cell Type"::Text);
    ...
    begin
        ...
        GSTR1EXPQuery.Open();
        while GSTR1EXPQuery.Read()do FillExcelBufferForExport(GSTR1ExpQuery);
    end;
    ...
    begin
        TempExcelBuffer.NewRow();
        if GSTR1ExpQuery.GST_Without_Payment_of_Duty then TempExcelBuffer.AddColumn(UpperCase(WOPAYTxt), false, '', false, false, false, '', TempExcelBuffer."Cell Type"::Text)
        else
        ...
        if GSTR1ExpQuery.Finance_Charge_Memo then TempExcelBuffer.AddColumn(GetInvoiceValueFinCharge(GSTR1ExpQuery.Document_No_), false, '', false, false, false, '0.00', TempExcelBuffer."Cell Type"::Number)
        else
        ...
        while GSTR1ExpPerQuery.Read()do TempExcelBuffer.AddColumn(Abs(GSTR1ExpPerQuery.GST__), false, '', false, false, false, '', TempExcelBuffer."Cell Type"::Number);
        if GSTR1ExpQuery.GST_Jurisdiction_Type = GSTR1ExpQuery.GST_Jurisdiction_Type::Intrastate then TempExcelBuffer.AddColumn(Abs(GSTR1ExpQuery.GST_Base_Amount / 2), false, '', false, false, false, '0.00', TempExcelBuffer."Cell Type"::Number)
        else
    ...
    local procedure GetExitPoint(DocumentNo: Code[20]): Code[10]var
        SalesInvoiceHeader: Record "Sales Invoice Header";
    begin
        if SalesInvoiceHeader.Get(DocumentNo)then exit(SalesInvoiceHeader."Exit Point");
    end;
    ...
    begin
        ...
        while GSTR1ATADJQuery.Read()do FillExcelBufferForATADJ(GSTR1ATADJQuery);
    end;
    ...
    begin
        TempExcelBuffer.NewRow();
        if State.Get(GSTR1ATADJQuery.Buyer_Seller_State_Code)then TempExcelBuffer.AddColumn(GSTR1ATADJQuery.State_Code__GST_Reg__No__ + '-' + GSTR1ATADJQuery.Description, false, '', false, false, false, '', TempExcelBuffer."Cell Type"::Text)
        else
            TempExcelBuffer.AddColumn('', false, '', false, false, false, '', TempExcelBuffer."Cell Type"::Text);
        ...
        if GSTR1ATADJGSTPer.Read()then if GSTR1ATADJGSTPer.GST_Jurisdiction_Type = GSTR1ATADJGSTPer.GST_Jurisdiction_Type::Intrastate then TempExcelBuffer.AddColumn(2 * GSTR1ATADJGSTPer.GST__, false, '', false, false, false, '', TempExcelBuffer."Cell Type"::Number)
            else
                TempExcelBuffer.AddColumn(GSTR1ATADJGSTPer.GST__, false, '', false, false, false, '', TempExcelBuffer."Cell Type"::Number);
        if GSTR1ATADJQuery.GST_Jurisdiction_Type = GSTR1ATADJQuery.GST_Jurisdiction_Type::Intrastate then TempExcelBuffer.AddColumn(Abs(GSTR1ATADJQuery.GST_Base_Amount / 2), false, '', false, false, false, '0.00', TempExcelBuffer."Cell Type"::Number)
        else
        ...
        if GSTR1ATADJCessAmt.Read()then TempExcelBuffer.AddColumn(Abs(GSTR1ATADJCessAmt.GST_Amount), false, '', false, false, false, '0.00', TempExcelBuffer."Cell Type"::Number)
        else
            TempExcelBuffer.AddColumn(0.00, false, '', false, false, false, '0.00', TempExcelBuffer."Cell Type"::Number);
    end;
    ...
    begin
        TempExcelBuffer.NewRow();
        if State.Get(GSTR1ATQuery.Buyer_Seller_State_Code)then TempExcelBuffer.AddColumn(GSTR1ATQuery.State_Code__GST_Reg__No__ + '-' + GSTR1ATQuery.Description, false, '', false, false, false, '', TempExcelBuffer."Cell Type"::Text)
        else
            TempExcelBuffer.AddColumn('', false, '', false, false, false, '', TempExcelBuffer."Cell Type"::Text);
        ...
        GSTR1ATPer.Open();
        while GSTR1ATPer.Read()do if GSTR1ATPer.GST_Jurisdiction_Type = GSTR1ATPer.GST_Jurisdiction_Type::Intrastate then TempExcelBuffer.AddColumn(2 * GSTR1ATPer.GST__, false, '', false, false, false, '', TempExcelBuffer."Cell Type"::Number)
            else
                TempExcelBuffer.AddColumn(GSTR1ATPer.GST__, false, '', false, false, false, '', TempExcelBuffer."Cell Type"::Number);
        if GSTR1ATQuery.GST_Jurisdiction_Type = GSTR1ATQuery.GST_Jurisdiction_Type::Intrastate then TempExcelBuffer.AddColumn(Abs(GSTR1ATQuery.GST_Base_Amount / 2), false, '', false, false, false, '0.00', TempExcelBuffer."Cell Type"::Number)
        else
        ...
        GSTR1ATCessAmt.Open();
        if GSTR1ATCessAmt.Read()then TempExcelBuffer.AddColumn(Abs(GSTR1ATCessAmt.GST_Amount), false, '', false, false, false, '0.00', TempExcelBuffer."Cell Type"::Number)
        else
            TempExcelBuffer.AddColumn(0.00, false, '', false, false, false, '0.00', TempExcelBuffer."Cell Type"::Number);
    end;
    ...
    begin
        ...
        GSTR1CDNRQuery.SetFilter(Document_Type, '%1|%2|%3', "GST Document Type"::"Credit Memo", "GST Document Type"::Invoice, "GST Document Type"::Refund);
        GSTR1CDNRQuery.SetFilter(GST_Customer_Type, '%1|%2|%3|%4', "GST Customer Type"::"Deemed Export", "GST Customer Type"::"SEZ Development", "GST Customer Type"::"SEZ Unit", "GST Customer Type"::Registered);
        GSTR1CDNRQuery.Open();
        while GSTR1CDNRQuery.Read()do if FilterDGLEForCDNR(GSTR1CDNRQuery)then FillExcelBufferForCDNR(GSTR1CDNRQuery);
    end;
    local procedure FilterDGLEForCDNR(GSTR1CDNRQuery: Query GSTR1CDNRQuery): Boolean begin
        if FilterDGLEDocTypeForCDNR(GSTR1CDNRQuery) or FilterDGLEDocTypeInvForCDNR(GSTR1CDNRQuery)then exit(true);
    end;
    local procedure FilterDGLEDocTypeForCDNR(GSTR1CDNRQuery: Query GSTR1CDNRQuery): Boolean begin
        if(GSTR1CDNRQuery.Document_Type = GSTR1CDNRQuery.Document_Type::"Credit Memo") or (FilterDGLEForRefundDocType(GSTR1CDNRQuery))then exit(true);
    end;
    local procedure FilterDGLEForRefundDocType(GSTR1CDNRQuery: Query GSTR1CDNRQuery): Boolean begin
        if(GSTR1CDNRQuery.Document_Type = GSTR1CDNRQuery.Document_Type::Refund) and (not GSTR1CDNRQuery.Adv__Pmt__Adjustment)then exit(true);
    end;
    local procedure FilterDGLEDocTypeInvForCDNR(GSTR1CDNRQuery: Query GSTR1CDNRQuery): Boolean begin
        if(GSTR1CDNRQuery.Document_Type = GSTR1CDNRQuery.Document_Type::Invoice) and FilterDGLESalesInvForCDNR(GSTR1CDNRQuery)then exit(true);
    end;
    ...
    begin
        ...
        TempExcelBuffer.AddColumn('', false, '', false, false, false, '', TempExcelBuffer."Cell Type"::Text);
        if GSTR1CDNRQuery.Buyer_Seller_State_Code <> '' then TempExcelBuffer.AddColumn(GSTR1CDNRQuery.State_Code__GST_Reg__No__ + '-' + GSTR1CDNRQuery.Description, false, '', false, false, false, '', TempExcelBuffer."Cell Type"::Text)
        else
            TempExcelBuffer.AddColumn('', false, '', false, false, false, '', TempExcelBuffer."Cell Type"::Text);
        if GSTR1CDNRQuery.Document_Type in[GSTR1CDNRQuery.Document_Type::Invoice, GSTR1CDNRQuery.Document_Type::"Credit Memo"]then if GSTR1CDNRQuery.Finance_Charge_Memo then TempExcelBuffer.AddColumn(GetInvoiceValueFinCharge(GSTR1CDNRQuery.Document_No_), false, '', false, false, false, '0.00', TempExcelBuffer."Cell Type"::Number)
            else
        ...
        while GSTR1CDNRPerQuery.Read()do if GSTR1CDNRPerQuery.GST_Jurisdiction_Type = GSTR1CDNRPerQuery.GST_Jurisdiction_Type::Intrastate then TempExcelBuffer.AddColumn(2 * GSTR1CDNRPerQuery.GST__, false, '', false, false, false, '', TempExcelBuffer."Cell Type"::Number)
            else
                TempExcelBuffer.AddColumn(GSTR1CDNRPerQuery.GST__, false, '', false, false, false, '', TempExcelBuffer."Cell Type"::Number);
        if GSTR1CDNRQuery.GST_Jurisdiction_Type = GSTR1CDNRQuery.GST_Jurisdiction_Type::Intrastate then TempExcelBuffer.AddColumn(Abs(GSTR1CDNRQuery.GST_Base_Amount / 2), false, '', false, false, false, '0.00', TempExcelBuffer."Cell Type"::Number)
        else
        ...
        GSTR1CDNRCess.Open();
        if GSTR1CDNRCess.Read()then TempExcelBuffer.AddColumn(Abs(GSTR1CDNRCess.GST_Amount), false, '', false, false, false, '0.00', TempExcelBuffer."Cell Type"::Number)
        else
            TempExcelBuffer.AddColumn(0.00, false, '', false, false, false, '0.00', TempExcelBuffer."Cell Type"::Number);
        if CheckPreGSTForCDNR(GSTR1CDNRQuery)then TempExcelBuffer.AddColumn(YLbl, false, '', false, false, false, '', TempExcelBuffer."Cell Type"::Text)
        else
            TempExcelBuffer.AddColumn(NLbl, false, '', false, false, false, '', TempExcelBuffer."Cell Type"::Text);
    end;
    ...
    begin
        ...
        if CustLedgerEntry.FindFirst()then PostingDate:=CustLedgerEntry."Posting Date";
        exit(PostingDate);
    end;
    ...
    begin
        ...
        GSTR1CDNURQuery.SetFilter(Document_Type, '%1|%2|%3', "GST Document Type"::"Credit Memo", "GST Document Type"::Invoice, "GST Document Type"::Refund);
        GSTR1CDNURQuery.SetFilter(GST_Customer_Type, '%1|%2', "GST Customer Type"::Export, "GST Customer Type"::Unregistered);
        GSTR1CDNURQuery.Open();
        while GSTR1CDNURQuery.Read()do if FilterDGLEForCDNUR(GSTR1CDNURQuery)then MakeExcelBodyLinesCDNUR(GSTR1CDNURQuery);
    end;
    ...
    begin
        ...
        TempExcelBuffer.AddColumn('', false, '', false, false, false, '', TempExcelBuffer."Cell Type"::Text);
        if GSTR1CDNURQuery.Buyer_Seller_State_Code <> '' then TempExcelBuffer.AddColumn(GetStateCodeGSTRegForCDNUR(GSTR1CDNURQuery) + '-' + GetStateDesForCDNUR(GSTR1CDNURQuery), false, '', false, false, false, '', TempExcelBuffer."Cell Type"::Text)
        else
        ...
        if GSTR1CDNURQuery.Document_Type in[GSTR1CDNURQuery.Document_Type::"Credit Memo", GSTR1CDNURQuery.Document_Type::Invoice]then if GSTR1CDNURQuery.Finance_Charge_Memo then TempExcelBuffer.AddColumn(GetInvoiceValueFinCharge(GSTR1CDNURQuery.Document_No_), false, '', false, false, false, '0.00', TempExcelBuffer."Cell Type"::Number)
            else
        ...
        while GSTR1CDNURGSTPer.Read()do if GSTR1CDNURGSTPer.GST_Jurisdiction_Type = GSTR1CDNURGSTPer.GST_Jurisdiction_Type::Intrastate then TempExcelBuffer.AddColumn(2 * GSTR1CDNURGSTPer.GST__, false, '', false, false, false, '', TempExcelBuffer."Cell Type"::Number)
            else
        ...
        if GSTR1CDNURCessAmt.Read()then TempExcelBuffer.AddColumn(Abs(GSTR1CDNURCessAmt.GST_Amount), false, '', false, false, false, '0.00', TempExcelBuffer."Cell Type"::Number)
        else
            TempExcelBuffer.AddColumn(0.00, false, '', false, false, false, '0.00', TempExcelBuffer."Cell Type"::Number);
        if CheckPreGSTForCDNUR(GSTR1CDNURQuery)then TempExcelBuffer.AddColumn(YLbl, false, '', false, false, false, '', TempExcelBuffer."Cell Type"::Text)
        else
            TempExcelBuffer.AddColumn(NLbl, false, '', false, false, false, '', TempExcelBuffer."Cell Type"::Text);
    end;
    local procedure FilterDGLEForCDNUR(GSTR1CDNURQuery: Query GSTR1CDNURQuery): Boolean begin
        if FilterDGLECustTypeForCDNUR(GSTR1CDNURQuery) and (FilterDGLEDocTypeForCDNUR(GSTR1CDNURQuery) or FilterDGLEDocTypeInvForCDNUR(GSTR1CDNURQuery))then exit(true);
    end;
    local procedure FilterDGLECustTypeForCDNUR(GSTR1CDNURQuery: Query GSTR1CDNURQuery): Boolean begin
        if(GSTR1CDNURQuery.GST_Customer_Type = GSTR1CDNURQuery.GST_Customer_Type::Export) or (FilterDGLEUnregCustForCDNUR(GSTR1CDNURQuery))then exit(true);
    end;
    local procedure FilterDGLEUnregCustForCDNUR(GSTR1CDNURQuery: Query GSTR1CDNURQuery): Boolean begin
        if(GSTR1CDNURQuery.GST_Customer_Type = GSTR1CDNURQuery.GST_Customer_Type::Unregistered) and (GetInvoiceValue(GSTR1CDNURQuery.Document_No_, "GST Document Type"::"Credit Memo") >= B2CLimit)then exit(true);
    end;
    local procedure FilterDGLEDocTypeForCDNUR(GSTR1CDNURQuery: Query GSTR1CDNURQuery): Boolean begin
        if(GSTR1CDNURQuery.Document_Type = GSTR1CDNURQuery.Document_Type::"Credit Memo")then exit(true);
    end;
    local procedure FilterDGLEDocTypeInvForCDNUR(GSTR1CDNURQuery: Query GSTR1CDNURQuery): Boolean begin
        if(GSTR1CDNURQuery.Document_Type = GSTR1CDNURQuery.Document_Type::Invoice) and (FilterDGLESalesInvForCDNUR(GSTR1CDNURQuery))then exit(true);
    end;
    local procedure FilterDGLESalesInvForCDNUR(GSTR1CDNURQuery: Query GSTR1CDNURQuery): Boolean begin
        if(GSTR1CDNURQuery.Sales_Invoice_Type in[GSTR1CDNURQuery.Sales_Invoice_Type::"Debit Note", GSTR1CDNURQuery.Sales_Invoice_Type::Supplementary])then exit(true);
    end;
    local procedure FilterDGLESalesInvForCDNR(GSTR1CDNRQuery: Query GSTR1CDNRQuery): Boolean begin
        if(GSTR1CDNRQuery.Sales_Invoice_Type in[GSTR1CDNRQuery.Sales_Invoice_Type::"Debit Note", GSTR1CDNRQuery.Sales_Invoice_Type::Supplementary])then exit(true);
    end;
    local procedure GetStateCodeGSTRegForCDNUR(GSTR1CDNURQuery: Query GSTR1CDNURQuery): Code[10]var
        State: Record State;
        StateGSTRegNo: Code[10];
    begin
        State.SetRange(Code, GSTR1CDNURQuery.Buyer_Seller_State_Code);
        if State.FindFirst()then StateGSTRegNo:=State."State Code (GST Reg. No.)";
        exit(StateGSTRegNo)end;
    local procedure GetStateDesForCDNUR(GSTR1CDNURQuery: Query GSTR1CDNURQuery): Text[50]var
        State: Record State;
        Description: Text[50];
    begin
        State.SetRange(Code, GSTR1CDNURQuery.Buyer_Seller_State_Code);
        if State.FindFirst()then Description:=State.Description;
        exit(Description);
    end;
    local procedure GetLocStateCodeRegForCDNUR(GSTR1CDNURQuery: Query GSTR1CDNURQuery): Code[10]var
        State: Record State;
        StateGSTRegNo: Code[10];
    begin
        ...
        if State.FindFirst()then StateGSTRegNo:=State."State Code (GST Reg. No.)";
        exit(StateGSTRegNo)end;
    local procedure GetLocStateDesForCDNUR(GSTR1CDNURQuery: Query GSTR1CDNURQuery): Text[50]var
        State: Record State;
        Description: Text[50];
    begin
        State.SetRange(Code, GSTR1CDNURQuery.Location_State_Code);
    ...
    local procedure GetDocumentTypeTxt(GSTDocumentType: Enum "GST Document Type"): Text begin
        case GSTDocumentType of GSTDocumentType::"Credit Memo": exit(CLbl);
        GSTDocumentType::Invoice: exit(DLbl);
        GSTDocumentType::Refund: exit(RLbl);
        end;
    end;
    local procedure CheckPreGSTForCDNR(GSTR1CDNRQuery: Query GSTR1CDNRQuery): Boolean var
        ReferenceInvoiceNo: Record "Reference Invoice No.";
    begin
        ReferenceInvoiceNo.Reset();
        ReferenceInvoiceNo.SetRange("Document No.", GSTR1CDNRQuery.Document_No_);
        ReferenceInvoiceNo.SetRange("Document Type", GSTDocumentType2DocumentTypeEnum(GSTR1CDNRQuery.Document_Type));
        ReferenceInvoiceNo.SetRange("Source No.", GSTR1CDNRQuery.Source_No_);
        if not ReferenceInvoiceNo.IsEmpty then exit(true);
    end;
    local procedure CheckPreGSTForCDNUR(GSTR1CDNURQuery: Query GSTR1CDNURQuery): Boolean var
        ReferenceInvoiceNo: Record "Reference Invoice No.";
    begin
        ReferenceInvoiceNo.Reset();
        ReferenceInvoiceNo.SetRange("Document No.", GSTR1CDNURQuery.Document_No_);
    ...
    local procedure GetURType(GSTR1CDNURQuery: Query GSTR1CDNURQuery): Text begin
        case GSTR1CDNURQuery.GST_Customer_Type of GSTR1CDNURQuery.GST_Customer_Type::Unregistered: exit(B2CLTxt);
        GSTR1CDNURQuery.GST_Customer_Type::"SEZ Development", GSTR1CDNURQuery.GST_Customer_Type::"SEZ Unit", GSTR1CDNURQuery.GST_Customer_Type::Export, GSTR1CDNURQuery.GST_Customer_Type::"Deemed Export": begin
            if GSTR1CDNURQuery.GST_Without_Payment_of_Duty then exit(UpperCase(EXPWOPayTxt));
            exit(UpperCase(EXPWPayTxt));
        end;
        end;
    end;
    local procedure GetInvoiceType(GSTR1B2BQuery: Query GSTR1B2BQuery): Text[50]begin
        case GSTR1B2BQuery.GST_Customer_Type of GSTR1B2BQuery.GST_Customer_Type::Registered: exit(RegularTxt);
        GSTR1B2BQuery.GST_Customer_Type::"SEZ Development", GSTR1B2BQuery.GST_Customer_Type::"SEZ Unit": begin
            if GSTR1B2BQuery.GST_Without_Payment_of_Duty then exit(SEZWOPayTxt);
            exit(SEZWPayTxt);
        end;
        GSTR1B2BQuery.GST_Customer_Type::"Deemed Export": exit(DeemedExportTxt);
        end;
    end;
    ...
    begin
        ...
        if CustLedgerEntry.FindFirst()then CustLedgerEntry.CalcFields("Amount (LCY)");
        exit(Abs(CustLedgerEntry."Amount (LCY)"));
    end;
    local procedure GSTDocumentType2GenJnlDocumentType(GSTDocumentType: Enum "GST Document Type"): Enum "Gen. Journal Document Type" begin
        case GSTDocumentType of GSTDocumentType::" ": exit("Gen. Journal Document Type"::" ");
        GSTDocumentType::Payment: exit("Gen. Journal Document Type"::Payment);
        GSTDocumentType::Invoice: exit("Gen. Journal Document Type"::Invoice);
        GSTDocumentType::"Credit Memo": exit("Gen. Journal Document Type"::"Credit Memo");
        GSTDocumentType::Refund: exit("Gen. Journal Document Type"::Refund);
        end;
    end;
    local procedure GetDocumentDate(DocumentNo: Code[20]; DocumentType: Enum "GST Document Type"): Date var
        CustLedgerEntry: Record "Cust. Ledger Entry";
    begin
        CustLedgerEntry.SetRange("Document Type", GSTDocumentType2GenJnlDocumentType(DocumentType));
        CustLedgerEntry.SetRange("Document No.", DocumentNo);
        if CustLedgerEntry.FindFirst()then exit(CustLedgerEntry."Document Date");
    end;
