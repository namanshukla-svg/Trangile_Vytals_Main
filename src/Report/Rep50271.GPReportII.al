Report 50271 "GP Report II"
{
    ...
    dataset
    {
        dataitem("Sales Invoice Header"; "Sales Invoice Header")
        {
            ...
            dataitem("Sales Invoice Line"; "Sales Invoice Line")
            {
                ...
                column(Sales_Invoice_Line__Sales_Invoice_Line___Unit_Price_; "Sales Invoice Line"."Unit Price" * SIExchangeRate)
                {
                }
                column(UnitCost; UnitCost)
                {
                }
                column(Sales_Invoice_Line__Sales_Invoice_Line___Line_Amount_; "Sales Invoice Line"."Line Amount" * SIExchangeRate)
                {
                }
                column(COGS; COGS * SIExchangeRate)
                {
                }
                column(Profit; Abs(Profit * SIExchangeRate))
                {
                }
            ...
            trigger OnAfterGetRecord()
            begin
                Clear(SIExchangeRate);
                if "Sales Invoice Header"."Currency Code" <> '' then SIExchangeRate:=1 / "Sales Invoice Header"."Currency Factor"
                else
                    SIExchangeRate:=1;
            end;
        }
        dataitem("Sales Cr.Memo Header"; "Sales Cr.Memo Header")
        {
            ...
            dataitem("Sales Cr.Memo Line"; "Sales Cr.Memo Line")
            {
                ...
                column(SCML_Unit_Price; "Sales Cr.Memo Line"."Unit Price" * CNExchangeRate)
                {
                }
                column(SCML_Unit_Cost_LCY; "Sales Cr.Memo Line"."Unit Cost (LCY)")
                {
                }
                column(SCML_Line_Amt; "Sales Cr.Memo Line".Amount * CNExchangeRate)
                {
                }
                ...
                column(COGSCR; COGSCR * CNExchangeRate)
                {
                }
            ...
            trigger OnAfterGetRecord()
            begin
                Clear(CNExchangeRate);
                if "Sales Cr.Memo Header"."Currency Code" <> '' then CNExchangeRate:=1 / "Sales Cr.Memo Header"."Currency Factor"
                else
                    CNExchangeRate:=1;
            end;
        }
    ...
    Profit_CaptionLbl: label 'Profit (LCY)';
    COGSCaptionLbl: label 'COGS (LCY)';
    Line_AmountCaptionLbl: label 'Line Amount (LCY)';
    Unit_Cost_LCY_CaptionLbl: label 'Unit Cost(LCY)';
    Unit_PriceCaptionLbl: label 'Unit Price (LCY)';
    COGSCR: Decimal;
    ProfitPer1: Decimal;
    ItemNo: Code[10];
    SIExchangeRate: Decimal;
    CNExchangeRate: Decimal;
}
