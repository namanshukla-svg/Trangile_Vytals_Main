Report 50488 "Sales Invoice new_ with LOT"
{
    DefaultLayout = RDLC;
    RDLCLayout = './Layouts/Sales Invoice new_ with LOT.rdl';
                ...
                dataitem(PageLoop; "Integer")
                {
                    ...
                    dataitem("Sales Invoice Line"; "Sales Invoice Line")
                    {
                        ...
                        begin
                            ...
                            SalesShipmentHeaderRec.SetRange("Order/Scd. No.", "Sales Invoice Header"."Order/Scd. No.");
                            SalesShipmentHeaderRec.SetRange("Location Code", "Sales Invoice Header"."Location Code");
                            if SalesShipmentHeaderRec.findset() then
                                repeat
                                    SalesShipmentLineRec.Reset();
                                    SalesShipmentLineRec.setrange("Document No.", SalesShipmentHeaderRec."No.");
                                    SalesShipmentLineRec.setrange("No.", "Sales Invoice Line"."No.");
                                    if SalesShipmentLineRec.findset() then
                                        repeat
                                            ...
                                            if ItemLedgerEntryRec.FindSet() then
                                                repeat
                                                    if LotNo = '' then
                                                        LotNo := Format(ItemLedgerEntryRec."Lot No.")
                                                    else
                                                        LotNo += ', ' + Format(ItemLedgerEntryRec."Lot No.");
