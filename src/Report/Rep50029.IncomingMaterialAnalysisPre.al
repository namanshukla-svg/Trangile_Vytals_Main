Report 50029 "Incoming Material Analysis-Pre"
{
    DefaultLayout = RDLC;
    RDLCLayout = './Layouts/Incoming Material Analysis-Pre.rdl';
    Caption = '<Incoming Material Analysis-Pre>';
    UsageCategory = Lists;
    ApplicationArea = all;
    ...
    dataset
    {
        dataitem("Quality Order Header"; "SSD Quality Order Header")
        {
            ...
            column(ReportForNavId_1494;1494)
            {
            }
            ...
            column(Quality_Order_Header__No__; "No.")
            {
            }
            column(Quality_Order_Header__Posting_Date_; "Posting Date")
            {
            }
            column(Quality_Order_Header__Item_No__; "Item No.")
            {
            }
            column(Item_Description_____Item__Description_2_; Item.Description + ' ' + Item."Description 2")
            {
            }
            column(Quality_Order_Header__Sampling_Temp__No__; "Sampling Temp. No.")
            {
            }
            column(Quality_Order_Header__Lot_No__; "Lot No.")
            {
            }
            ...
            column(Quality_Order_Header__Quality_Order_Header___Order_No__; "Quality Order Header"."Order No.")
            {
            }
            ...
            column(VendorItemDescription_QualityOrderHeader; "Vendor Item Description")
            {
            }
            dataitem("Quality Order Line"; "SSD Quality Order Line")
            {
                DataItemLink = "Document No."=field("No.");
                DataItemTableView = sorting("Document No.", "Line No.");

                column(ReportForNavId_8806;8806)
                {
                }
                column(Quality_Order_Line_Description; Description)
                {
                }
                ...
                column(Quality_Order_Line__Quality_Order_Line___Unit_of_Measure_Code_; "Quality Order Line"."Unit of Measure Code")
                {
                }
                ...
                column(Quality_Order_Line__Quality_Order_Line___Unit_of_Measure_Code__Control1000000075; "Quality Order Line"."Unit of Measure Code")
                {
                }
                column(QualityMeasurements_Protocol_Control1000000076; QualityMeasurements.Protocol)
                {
                }
                column(Quality_Order_Line_Description_Control1000000077; Description)
                {
                }
                column(Quality_Order_Header___Accepted_Qty__; "Quality Order Header"."Accepted Qty.")
                {
                }
                column(Quality_Order_Header___Rejected_Qty__; "Quality Order Header"."Rejected Qty.")
                {
                }
                column(Quality_Order_Header__Remarks; "Quality Order Header".Remarks)
                {
                }
                ...
                column(Quality_Order_Line_Document_No_; "Document No.")
                {
                }
                column(Quality_Order_Line_Line_No_; "Line No.")
                {
                }
                trigger OnAfterGetRecord()
                begin
                    if "Quality Order Line"."Meas. Value Type" = "Quality Order Line"."meas. value type"::Value then begin
                        AverageTestValue:=Format(ROUND("Quality Order Line"."Inspection Value2"));
                    end
                    else
                    begin
                        if "Quality Order Line"."Quality Pass" then AverageTestValue:='PASS'
                        else
                            AverageTestValue:='NOT PASS' end;
                    Item.Reset;
                    if Item.Get("Quality Order Header"."Item No.")then;
                    PurchaseOrder.Reset;
                    Vendor.Reset;
                    if PurchaseOrder.Get(PurchaseOrder."document type"::Order, "Quality Order Header"."Order No.")then begin
                        if Vendor.Get(PurchaseOrder."Buy-from Vendor No.")then;
                    end
                    else
                    begin
                        if PurchaseOrder.Get(PurchaseOrder."document type"::Order, "Quality Order Header"."Source Document No.")then if Vendor.Get(PurchaseOrder."Buy-from Vendor No.")then;
                    end;
                    PurchRcptLine.Reset;
                    PurchRcptLine.SetRange(PurchRcptLine."No.", "Quality Order Header"."Item No.");
                    PurchRcptLine.SetRange(PurchRcptLine."Posted Quality Order No.", "Quality Order Header"."No.");
                    if PurchRcptLine.FindFirst then begin
                        if PurchRcptHdr.Get(PurchRcptLine."Document No.")then begin
                            RcptNo:=PurchRcptHdr."No.";
                            RcptDate:=PurchRcptHdr."Posting Date";
                            VendInvNo:=PurchRcptHdr."Vendor Shipment No.";
                        end;
                    end;
                    if "Quality Order Line"."Minimum Value" <> 0 then MinVal:=Format("Quality Order Line"."Minimum Value")
                    else
                        MinVal:='-';
                    if "Quality Order Line"."Maximum Value" <> 0 then MaxVal:=Format("Quality Order Line"."Maximum Value")
                    else
                        MaxVal:='-';
                    QualityMeasurements.Reset;
                    if QualityMeasurements.Get("Quality Order Line"."Meas. Code")then;
                end;
            }
        }
    }
    ...
    begin
        FixedText:='FM-QA-01, Rev.No.01, Effective Date 13/01/2012';
    end;
    trigger OnPreReport()
    begin
        CompInfo.Get;
    ...
    Incoming_Material_Analysis_ReportCaptionLbl: label 'Incoming Material Analysis Report';
