Report 50500 "SSD Customer - Order Summary"
{
    DefaultLayout = RDLC;
    RDLCLayout = './Layouts/Customer - Order Summary.rdl';
    ...
    dataset
    {
        dataitem(Customer; Customer)
        {
            ...
            begin
                ...
                if SalesLine.FindSet then repeat PeriodNo:=1;
                        while SalesLine."Shipment Date" >= PeriodStartDate[PeriodNo]do PeriodNo:=PeriodNo + 1;
                        Currency.InitRoundingPrecision;
                        if SalesLine."VAT Calculation Type" in[SalesLine."vat calculation type"::"Normal VAT", SalesLine."vat calculation type"::"Reverse Charge VAT"]then SalesOrderAmount:=ROUND((SalesLine.Amount + SalesLine."VAT Base Amount" * SalesLine."VAT %" / 100) * SalesLine."Outstanding Quantity" / SalesLine.Quantity / (1 + SalesLine."VAT %" / 100), Currency."Amount Rounding Precision")
                        else
                            SalesOrderAmount:=ROUND(SalesLine."Outstanding Amount" / (1 + SalesLine."VAT %" / 100), Currency."Amount Rounding Precision");
                        SalesOrderAmountLCY:=SalesOrderAmount;
                        if SalesLine."Currency Code" <> '' then begin
                            SalesHeader.Get(SalesLine."Document Type", SalesLine."Document No.");
                            if SalesHeader."Currency Factor" <> 0 then SalesOrderAmountLCY:=ROUND(CurrExchRate.ExchangeAmtFCYToLCY(WorkDate, SalesLine."Currency Code", SalesOrderAmount, SalesHeader."Currency Factor"));
                        end;
    ...
    begin
        ...
        if not PrintAmountsInLCY then begin
            ...
            if Currency.Count = 1 then Currency.FindFirst;
        end;
