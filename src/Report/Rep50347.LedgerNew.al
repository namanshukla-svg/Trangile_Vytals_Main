Report 50347 "Ledger New"
{
    DefaultLayout = RDLC;
    RDLCLayout = './Layouts/Ledger.rdl';
    Caption = 'Ledger New';
        ...
        dataitem("G/L Account"; "G/L Account")
        {
            ...
            dataitem("G/L Entry"; "G/L Entry")
            {
                ...
                dataitem("Integer"; "Integer")
                {
                    ...
                    begin
                        ...
                        if FirstRecord and (ControlAccountName <> '')then begin
                            DetailAmt:=0;
                            if PrintDetail then DetailAmt:=GLEntry.Amount;
                            ...
                            DrCrTextBalance:='';
                            if OpeningDRBal - OpeningCRBal + TransDebits - TransCredits > 0 then DrCrTextBalance:='Dr';
                            if OpeningDRBal - OpeningCRBal + TransDebits - TransCredits < 0 then DrCrTextBalance:='Cr';
                        end
                        else if FirstRecord and (ControlAccountName = '')then begin
                                DetailAmt:=0;
                                if PrintDetail then DetailAmt:=GLEntry.Amount;
                                if DetailAmt > 0 then DrCrText:='Dr';
                                if DetailAmt < 0 then DrCrText:='Cr';
                        ...
                        if PrintDetail and (not FirstRecord)then begin
                            if GLEntry.Amount > 0 then DrCrText:='Dr';
                            if GLEntry.Amount < 0 then DrCrText:='Cr';
                            AccountName:=Daybook.FindGLAccName(GLEntry."Source Type", GLEntry."Entry No.", GLEntry."Source No.", GLEntry."G/L Account No.");
                        end;
                    end;
                    ...
                    begin
                        ...
                        FirstRecord:=true;
                        if GLEntry.Count = 1 then CurrReport.Break;
                    end;
                }
                ...
                dataitem(PostedNarration1; "Posted Narration")
                {
                    ...
                    trigger OnPreDataItem()
                    begin
                        if not PrintVchNarration then CurrReport.Break;
                        GLEntry2.Reset;
                        ...
                        GLEntry2.FindLast;
                        if not(GLEntry2."Entry No." = "G/L Entry"."Entry No.")then CurrReport.Break;
                    end;
                }
                trigger OnAfterGetRecord()
                begin
                    ...
                    OneEntryRecord:=true;
                    if GLEntry.Count > 1 then OneEntryRecord:=false;
                    GLAcc.Get("G/L Account No.");
                    ControlAccount:=FindControlAccount("Source Type", "Entry No.", "Source No.", "G/L Account No.");
                    if ControlAccount then ControlAccountName:=Daybook.FindGLAccName("Source Type", "Entry No.", "Source No.", "G/L Account No.");
                    if Amount > 0 then TransDebits:=TransDebits + Amount;
                    if Amount < 0 then TransCredits:=TransCredits - Amount;
                    SourceDesc:='';
                    ...
                    else if OneEntryRecord and (ControlAccountName = '')then begin
                            AccountName:=Daybook.FindGLAccName(GLEntry."Source Type", GLEntry."Entry No.", GLEntry."Source No.", GLEntry."G/L Account No.");
                            DrCrTextBalance:='';
                            if OpeningDRBal - OpeningCRBal + TransDebits - TransCredits > 0 then DrCrTextBalance:='Dr';
                            if OpeningDRBal - OpeningCRBal + TransDebits - TransCredits < 0 then DrCrTextBalance:='Cr';
                        end;
                    if GLAccountNo <> "G/L Account"."No." then GLAccountNo:="G/L Account"."No.";
                    if GLAccountNo = "G/L Account"."No." then begin
                        DrCrTextBalance2:='';
                        if OpeningDRBal - OpeningCRBal + TransDebits - TransCredits > 0 then DrCrTextBalance2:='Dr';
                        if OpeningDRBal - OpeningCRBal + TransDebits - TransCredits < 0 then DrCrTextBalance2:='Cr';
                    end;
            ...
            begin
                if AccountNo <> "No." then begin
                    ...
                    OpeningCRBal:=0;
                    GLEntry2.Reset;
                    GLEntry2.SetCurrentkey("G/L Account No.", "Business Unit Code", "Global Dimension 1 Code", "Global Dimension 2 Code", "Close Income Statement Dim. ID", "Posting Date"); //Location Code field is removed//SSD Gurmeet
                    GLEntry2.SetRange("G/L Account No.", "No.");
                    GLEntry2.SetFilter("Posting Date", '%1..%2', 0D, ClosingDate(GetRangeMin("Date Filter") - 1));
                    if "Global Dimension 1 Filter" <> '' then GLEntry2.SetFilter("Global Dimension 1 Code", "Global Dimension 1 Filter");
                    if "Global Dimension 2 Filter" <> '' then GLEntry2.SetFilter("Global Dimension 2 Code", "Global Dimension 2 Filter");
                    // if LocationCode <> '' then
                    //     GLEntry2.SetFilter("Location Code", LocationCode);
                    GLEntry2.CalcSums(Amount);
                    if GLEntry2.Amount > 0 then OpeningDRBal:=GLEntry2.Amount;
                    if GLEntry2.Amount < 0 then OpeningCRBal:=-GLEntry2.Amount;
                    DrCrTextBalance:='';
                    if OpeningDRBal - OpeningCRBal > 0 then DrCrTextBalance:='Dr';
                    if OpeningDRBal - OpeningCRBal < 0 then DrCrTextBalance:='Cr';
                end;
            end;
            trigger OnPreDataItem()
            begin
                CurrReport.CreateTotals(TransDebits, TransCredits, "G/L Entry"."Debit Amount", "G/L Entry"."Credit Amount");
                if LocationCode <> '' then LocationFilter:='Location Code: ' + LocationCode;
            end;
        }
    }
    requestpage
    ...
    procedure FindControlAccount("Source Type": Option " ", Customer, Vendor, "Bank Account", "Fixed Asset"; "Entry No.": Integer; "Source No.": Code[20]; "G/L Account No.": Code[20]): Boolean var
    ...
    begin
        if "Source Type" = "source type"::Vendor then begin
            if VendLedgEntry.Get("Entry No.")then SubLedgerFound:=true;
        end;
        if "Source Type" = "source type"::Customer then begin
            if CustLedgEntry.Get("Entry No.")then SubLedgerFound:=true;
        end;
        if "Source Type" = "source type"::"Bank Account" then if BankLedgEntry.Get("Entry No.")then begin
                SubLedgerFound:=true;
            end;
        if "Source Type" = "source type"::"Fixed Asset" then begin
            ...
            if FALedgEntry.FindFirst then SubLedgerFound:=true;
        end;
        exit(SubLedgerFound);
    end;
}
