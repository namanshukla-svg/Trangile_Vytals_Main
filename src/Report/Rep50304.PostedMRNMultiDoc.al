Report 50304 "Posted MRN MultiDoc"
{
    DefaultLayout = RDLC;
    RDLCLayout = './Layouts/Posted MRN MultiDoc.rdl';
    Caption = 'MRN';
    ...
    dataset
    {
        dataitem("Posted Whse. Receipt Header"; "Posted Whse. Receipt Header")
        {
            ...
            column(DocumentNosText; DocumentNosText) { }
            ...
            column(BillNo_PostedWhseReceiptHeader; "Posted Whse. Receipt Header"."Bill No.")
            {
            }
            dataitem("Integer"; "Integer")
            {
                ...
                column(RespCent_Name; companyInfoRec.Name)
                {
                }
                column(DataItem1000000001; Location.Address + ',' + Location."Address 2" + ',' + Location.City + '-' + Location."Post Code" + ',' + StateName1 + 'TEL No. ' + Location."Phone No." + ' FAX No. ' + Location."Fax No.")
                {
                }
                ...
                dataitem("Posted Whse. Receipt Line"; "Posted Whse. Receipt Line")
                {
                    ...
                    DataItemTableView = sorting("No.", "Line No.") where("Posting Date" = filter(> 01));
                    ...
                    column(Item_No____________Description; "Item No." + '  ' + Description)
                    {
                    }
                    ...
                    column(Posted_Whse__Receipt_Line_No_; PurchRcptLine."Document No.")
                    {
                    }
                    ...
                    begin
                        ...
                        PostedWhseReceiptHeader.SetRange("No.", "Posted Whse. Receipt Line"."No.");
                        if PostedWhseReceiptHeader.FindFirst then;
                        PostedWhseReceiptHeader.CalcFields("Bill No.");
                        // if StrPos(PostedWhseReceiptHeader."Bill No.", BillNo) = 0 then
                        //     CurrReport.Skip;
                        ...
                        PurchRcptLine.Reset;
                        PurchRcptLine.SetRange("Posted Whse. Rcpt No.", "Posted Whse. Receipt Line"."No.");
            ...
            trigger OnAfterGetRecord()
            var
                PostedWarehouseRcptHdr: Record "Posted Whse. Receipt Header";
            begin
                StateName1 := '';
                if STATES.Get(Location."State Code") then StateName1 := STATES.Description;
                if Location.Get("Location Code") then begin
                    LocationText := Location.Name + Location.Address + Location."Address 2" + Location."Post Code" + StateName1 + Location."Fax No." + Location."Phone No.";
                end;
                GetLocation("Location Code");
                ...
                BillNo := DelChr("Posted Whse. Receipt Header"."Bill No.", '=', '.');
                //MESSAGE(BillNo);
                //IG_DS
                PostedWarehouseRcptHdr.Reset();
                PostedWarehouseRcptHdr.SetRange("Vendor Shipment No.", "Posted Whse. Receipt Header"."Vendor Shipment No.");
                if PostedWarehouseRcptHdr.FindFirst() then
                    repeat
                        if DocumentNosText = '' then
                            DocumentNosText := PostedWarehouseRcptHdr."No."
                        else
                            DocumentNosText += ', ' + PostedWarehouseRcptHdr."No.";
                    until PostedWarehouseRcptHdr.Next() = 0;
                //IG_DS
            end;
        }
    }
