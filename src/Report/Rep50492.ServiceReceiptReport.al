report 50492 "Service Receipt Report"
{
    DefaultLayout = RDLC;
    RDLCLayout = './Layouts/ServiceReceiptNew.rdl';
    Caption = 'Service Receipt Report';
    ...
    dataset
    {
        dataitem("Purch. Rcpt. Header"; "Purch. Rcpt. Header")
        {
            ...
            column(AssignedUserID; AssignedUserID)
            {
            }
            ...
            column(CreatedBy; GetFullName(SystemCreatedBy))
            {
            }
            ...
            column(DeductionRemarks; DeductionRemarks)
            {
            }
            dataitem("SSD Posted PO CheckList"; "SSD Posted PO CheckList")
            {
                ...
                column(CheckList_SSDPostedPOCheckList; "Check List")
                {
                }
            ...
            {
                DataItemTableView = where("Table ID" = const(Database::"Purch. Rcpt. Header"), "Document Type" = const(Quote));
                DataItemLink = "No." = field("No.");
                DataItemLinkReference = "Purch. Rcpt. Header";

                column(AttachmentName; "File Name")
                {
                }
                trigger OnAfterGetRecord()
                begin
            ...
            dataitem(CopyLoop; "Integer")
            {
                ...
                dataitem(PageLoop; "Integer")
                {
                    ...
                    dataitem("Purch. Rcpt. Line"; "Purch. Rcpt. Line")
                    {
                        ...
                        column(SSD_Receipt_Remarks; "SSD Receipt Remarks")
                        {
                        }
                        ...
                        begin
                            ...
                            SrNo1 += 1;
                            LineAmount := Quantity * "Direct Unit Cost";
                        end;
                ...
                begin
                    if not IsReportInPreviewMode() then CODEUNIT.Run(CODEUNIT::"Purch.Rcpt.-Printed", "Purch. Rcpt. Header");
                end;
            ...
            trigger OnAfterGetRecord() //receipt header
            var
            begin
                Statecodevar := '';
                CheckText := '';
                CurrReport.Language := LanguageCode.GetLanguageIdOrDefault("Language Code");
                CurrReport.FormatRegion := LanguageCode.GetFormatRegionOrDefault("Format Region");
                ...
                if VendorRec.Get("Buy-from Vendor No.") then Statecodevar := VendorRec."State Code";
                SrNo1 := 0;
                DeductionAmount := 0;
                DeductionRemarks := '';
                PurchaseReceiptLine.Reset();
                PurchaseReceiptLine.SetRange("Document No.", "No.");
                if PurchaseReceiptLine.FindSet() then
                    repeat
                        DeductionAmount += PurchaseReceiptLine."SSD Deduction Amount";
                        if DeductionRemarks = '' then
                            DeductionRemarks := PurchaseReceiptLine."SSD Deduction Remarks"
                        else
                            DeductionRemarks += ',' + PurchaseReceiptLine."SSD Deduction Remarks";
                    until PurchaseReceiptLine.Next() = 0;
                if PurchaseHeaderRec.Get(PurchaseHeaderRec."Document Type"::Order, "Order No.") then AssignedUserID := PurchaseHeaderRec."Assigned User ID";
            end;
    ...
    begin
        if LogInteraction and not IsReportInPreviewMode() then
            if "Purch. Rcpt. Header".FindSet() then
                repeat
                    SegManagement.LogDocument(15, "Purch. Rcpt. Header"."No.", 0, 0, DATABASE::Vendor, "Purch. Rcpt. Header"."Buy-from Vendor No.", "Purch. Rcpt. Header"."Purchaser Code", '', "Purch. Rcpt. Header"."Posting Description", '');
    ...
    var
        ...
        // Language: Codeunit Language; //IG_DS_before
        LanguageCode: Codeunit Language;
    ...
    begin
        ...
        FormatAddr.PurchRcptBuyFrom(VendorAddress, PurchRcptHeader);
        // CompressArray(VendorAddress);
    end;
