Report 50360 "Dimensions - Total New"
{
    DefaultLayout = RDLC;
    RDLCLayout = './Layouts/Dimensions - Total.rdl';
    Caption = 'Dimensions - Total New';
    ApplicationArea = all;
    ...
    dataset
    {
        dataitem("Analysis View"; "Analysis View")
        {
            ...
            begin
                ...
                i:=1;
                if TempSelectedDim.Find('-')then begin
                    repeat DimCode[i]:=TempSelectedDim."Dimension Code";
                ...
                with ColLayoutTmp do begin
                    i:=0;
                    if Find('-')then begin
                        repeat if Show <> Show::Never then begin
                                i:=i + 1;
                                if i <= MaxColumnsDisplayed then begin
                                    Header[i]:="Column Header";
                                    RoundingHeader[i]:='';
                                    if "Rounding Factor" in["rounding factor"::"1000", "rounding factor"::"1000000"]then case "Rounding Factor" of "rounding factor"::"1000": RoundingHeader[i]:=Text006;
                                        "rounding factor"::"1000000": RoundingHeader[i]:=Text007;
                                        end;
                                end;
                            end;
                            NoOfCols:=NoOfCols + 1;
                ...
                if UseAmtsInAddCurr then HeaderText:=StrSubstNo(Text008, GLSetup."Additional Reporting Currency")
                else if GLSetup."LCY Code" <> '' then HeaderText:=StrSubstNo(Text008, GLSetup."LCY Code")
                    else
                        HeaderText:='';
                CurrReport.PageNo:=1;
            end;
                ...
                group(Options)
                {
                    ...
                    field(IncludeDimensions; ColumnDim)
                    {
                        ...
                        begin
                            ...
                            GetAccountSource;
                            if AccountSource = Accountsource::"G/L Account" then DimSelectionBuf.SetDimSelectionLevelGLAcc(3, Report::"Dimensions - Total New", AnalysisViewCode, ColumnDim)
                            else if AccountSource = Accountsource::"Cash Flow Account" then DimSelectionBuf.SetDimSelectionLevelCFAcc(3, Report::"Dimensions - Total New", AnalysisViewCode, ColumnDim);
                        end;
                    }
                    field(ColumnLayoutName; ColumnLayoutName)
                    {
                        ...
                        trigger OnValidate()
                        begin
                            if ColumnLayoutName <> '' then AccSchedManagement.CheckColumnName(ColumnLayoutName);
                        end;
                    }
    ...
    begin
        ...
        GetAccountSource;
        if ColumnLayoutName = '' then Error(Text001);
        if DateFilter = '' then Error(Text002);
        DimSelectionBuf.CompareDimText(3, Report::"Dimensions - Total New", AnalysisViewCode, ColumnDim, Text003);
        SelectedDim.GetSelectedDim(UserId, 3, Report::"Dimensions - Total New", AnalysisViewCode, TempSelectedDim);
        if AccountSource = Accountsource::"G/L Account" then FillTempGLAccount
        else if AccountSource = Accountsource::"Cash Flow Account" then FillTempCFAccount;
        TempSelectedDim.Reset;
        TempSelectedDim.SetCurrentkey("User ID", "Object Type", "Object ID", "Analysis View Code", Level);
        TempSelectedDim.SetFilter(Level, '<>%1', TempSelectedDim.Level::" ");
        if TempSelectedDim.FindSet then begin
            repeat TempDimVal.Init;
                TempDimVal.Code:='';
                ...
                if DimVal.FindSet then repeat TempDimVal.Init;
                        TempDimVal:=DimVal;
                        TempDimVal.Insert;
                    until DimVal.Next = 0;
            until TempSelectedDim.Next = 0;
    ...
    local procedure CalcLine(Level: Integer): Boolean var
        ...
        More: Boolean;
    begin
        if Iteration(FindFirstDim[Level], DimCode[Level], DimValCode[Level], DimValName[Level], LevelFilter[Level], Totaling, Indentation, PostingType)then begin
            if Level = 4 then LowestLevel:=true
            else
            ...
            if LowestLevel then DimValNameIndent[Level]:=Indentation
            else
            begin
                ...
                while i <= ArrayLen(DimCode)do begin
                    if DimCode[i] <> '' then if LevelFilter[i] <> '' then SetAccSchedLineFilter(DimCode[i], LevelFilter[i], true, '')
                        else
            ...
            if(not PrintEmptyLines) and LowestLevel and (PostingType = Postingtype::"Begin-Total")then begin
                ...
                while More and (not HasValue) and (ThisIndentation > Indentation)do begin
                    More:=Iteration(FindFirstDim[Level], DimCode[Level], ThisDimValCode, ThisDimValName, LevelFilter[Level], ThisTotaling, ThisIndentation, ThisPostingType);
                    if More then begin
                        SetAccSchedLineFilter(DimCode[Level], ThisDimValCode, true, ThisTotaling);
                        HasValue:=CalcColumns(0);
                    end;
    ...
    local procedure TestCalcLine(Level: Integer; ThisFindFirstDim: Boolean): Boolean var
        Totaling: Text[250];
        LowestLevel: Boolean;
        ThisDimValName: Text[100];
        ThisIndentation: Integer;
    ...
    begin
        ...
        while TryNext and not HasValue do begin
            ...
            if Iteration(FindFirstDim[Level], DimCode[Level], DimValCode[Level], ThisDimValName, LevelFilter[Level], Totaling, ThisIndentation, ThisPostingType)then begin
                if Level = 4 then LowestLevel:=true
                else
                    LowestLevel:=DimCode[Level + 1] = '';
                if LowestLevel then begin
                    ...
                    while More and (not HasValue)do begin
                        More:=Iteration(FindFirstDim[Level], DimCode[Level], DimValCode[Level], ThisDimValName, LevelFilter[Level], Totaling, ThisIndentation, ThisPostingType);
                        if More then begin
                            SetAccSchedLineFilter(DimCode[Level], DimValCode[Level], true, Totaling);
                            HasValue:=CalcColumns(0);
                        end;
    ...
    local procedure CalcColumns(Level: Integer): Boolean var
        NonZero: Boolean;
        i: Integer;
    begin
        NonZero:=false;
        if AccountSource = Accountsource::"G/L Account" then begin
            if not GLAccFilterSet then AccSchedLine.Totaling:=GLAccRange;
        end
        else if AccountSource = Accountsource::"Cash Flow Account" then if not CFAccFilterSet then AccSchedLine.Totaling:=CFAccRange;
        with ColLayoutTmp do begin
            SetRange("Column Layout Name", ColumnLayoutName);
            i:=0;
            if FindSet then repeat if Show <> Show::Never then begin
                        i:=i + 1;
                        AccSchedLine."Line No.":=AccSchedLine."Line No." + 1;
                        ColumnValuesDisplayed[i]:=AccSchedManagement.CalcCell(AccSchedLine, ColLayoutTmp, UseAmtsInAddCurr);
                        NonZero:=NonZero or (ColumnValuesDisplayed[i] <> 0) and ("Column Type" <> "column type"::Formula);
                        if Level > 0 then ColumnValuesAsText[i, Level]:=AccSchedManagement.FormatCellAsText(ColLayoutTmp, ColumnValuesDisplayed[i], UseAmtsInAddCurr);
                    end;
                until(i >= MaxColumnsDisplayed) or (Next() = 0);
        end;
        exit(NonZero);
    ...
    var
        ...
        AnalysisView: Record "Analysis View";
    begin
        AnalysisView.CopyAnalysisViewFilters(3, Report::"Dimensions - Total New", AnalysisViewCode);
        ColumnDim:='';
        SelectedDim.SetRange("User ID", UserId);
        SelectedDim.SetRange("Object Type", 3);
        SelectedDim.SetRange("Object ID", Report::"Dimensions - Total New");
    ...
    begin
        ...
        TempBusUnit.TableCaption: begin
            ...
            if FindFirst then SearchResult:=TempBusUnit.FindSet
        ...
        TempCFAccount.TableCaption: begin
            TempCFAccount.Reset();
            TempCFAccount.SetFilter("No.", IterationFilter);
            if FindFirst then SearchResult:=TempCFAccount.FindSet
            else if TempCFAccount.Get(IterationDimValCode)then SearchResult:=(TempCFAccount.Next <> 0);
            if SearchResult then begin
        ...
        TempCashFlowForecast.TableCaption: begin
            TempCashFlowForecast.Reset();
            TempCashFlowForecast.SetFilter("No.", IterationFilter);
            if FindFirst then SearchResult:=TempCashFlowForecast.FindSet
            else if TempCashFlowForecast.Get(IterationDimValCode)then SearchResult:=(TempCashFlowForecast.Next <> 0);
            if SearchResult then begin
                IterationDimValCode:=TempCashFlowForecast."No.";
                if TempCashFlowForecast."No." <> '' then IterationDimValName:=TempCashFlowForecast.Description
                else
                    IterationDimValName:=Text009;
                IterationIndentation:=0;
                IterationPostingType:=0;
        ...
        begin
            ...
            if FindFirst then SearchResult:=TempDimVal.FindSet
    ...
    var
        AnalysisView: Record "Analysis View";
    begin
        if CalledGetAccountSource then exit;
        if AnalysisView.Get(AnalysisViewCode)then begin
            AccountSource:=AnalysisView."Account Source";
            CalledGetAccountSource:=true;
        end;
        if AccountSource = Accountsource::"Cash Flow Account" then CashFlowEditable:=true
        else if AccountSource = Accountsource::"G/L Account" then CashFlowEditable:=false;
    end;
    ...
    begin
        ...
        TempSelectedDim.SetFilter("Dimension Value Filter", '<>%1', '');
        if TempSelectedDim.FindFirst then GLAcc.SetFilter("No.", TempSelectedDim."Dimension Value Filter");
        if GLAcc.FindSet then begin
            GLAccRange:=GLAcc."No.";
            repeat TempGLAcc.Init();
                TempGLAcc:=GLAcc;
                TempGLAcc.Insert();
            until GLAcc.Next() = 0;
        ...
        if TempSelectedDim.FindFirst then TempBusUnit.SetFilter(Code, TempSelectedDim."Dimension Value Filter");
        if BusUnit.FindSet then begin
            repeat TempBusUnit.Init();
                TempBusUnit:=BusUnit;
                TempBusUnit.Insert();
    ...
    begin
        ...
        TempSelectedDim.SetFilter("Dimension Value Filter", '<>%1', '');
        if TempSelectedDim.FindFirst then CFAccount.SetFilter("No.", TempSelectedDim."Dimension Value Filter");
        if CFAccount.FindSet then begin
            CFAccRange:=CFAccount."No.";
            repeat TempCFAccount.Init();
        ...
        if TempSelectedDim.FindFirst then TempCashFlowForecast.SetFilter("No.", TempSelectedDim."Dimension Value Filter");
        if CashFlowForecast.FindSet then begin
            repeat TempCashFlowForecast.Init();
    ...
    begin
        if Totaling <> '' then AnalysisViewFilter:=Totaling;
        if SetFilter and (AnalysisViewFilter = '')then AnalysisViewFilter:='''''';
        case AnalysisViewDimCode of TempGLAcc.TableCaption: begin
            GLAccFilterSet:=SetFilter;
            if SetFilter then AccSchedLine.Totaling:=AnalysisViewFilter
            else
                AccSchedLine.Totaling:=GLAccRange;
        end;
        TempBusUnit.TableCaption: if SetFilter then AccSchedLine.SetFilter("Business Unit Filter", AnalysisViewFilter)
            else
        ...
        TempCashFlowForecast.TableCaption: if SetFilter then AccSchedLine.SetFilter("Cash Flow Forecast Filter", AnalysisViewFilter)
            else
                AccSchedLine.SetRange("Cash Flow Forecast Filter");
        "Analysis View"."Dimension 1 Code": if SetFilter then AccSchedLine."Dimension 1 Totaling":=AnalysisViewFilter
            else
                AccSchedLine."Dimension 1 Totaling":='';
        "Analysis View"."Dimension 2 Code": if SetFilter then AccSchedLine."Dimension 2 Totaling":=AnalysisViewFilter
            else
                AccSchedLine."Dimension 2 Totaling":='';
        "Analysis View"."Dimension 3 Code": if SetFilter then AccSchedLine."Dimension 3 Totaling":=AnalysisViewFilter
            else
                AccSchedLine."Dimension 3 Totaling":='';
        "Analysis View"."Dimension 4 Code": if SetFilter then AccSchedLine."Dimension 4 Totaling":=AnalysisViewFilter
            else
    ...
    begin
        with AccSchedLine do begin
            ...
            case AccountSource of Accountsource::"G/L Account": begin
                "Totaling Type":="totaling type"::"Posting Accounts";
                if GLBudgetName <> '' then SetRange("G/L Budget Filter", GLBudgetName);
            end;
            Accountsource::"Cash Flow Account": begin
                "Totaling Type":="totaling type"::"Cash Flow Entry Accounts";
                if CashFlowForecastNo <> '' then SetRange("Cash Flow Forecast Filter", CashFlowForecastNo);
            end;
            end;
        end;
    end;
