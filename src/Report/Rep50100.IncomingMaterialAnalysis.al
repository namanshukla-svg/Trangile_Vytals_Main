Report 50100 "Incoming Material Analysis"
{
    DefaultLayout = RDLC;
    RDLCLayout = './Layouts/Incoming Material Analysis.rdl';
    ...
    dataset
    {
        dataitem("Posted Quality Order Header"; "SSD Posted Quality Order Hdr")
        {
            ...
            begin
                ...
                PurchaseOrder.Reset;
                Vendor.Reset;
                if PurchaseOrder.Get(PurchaseOrder."document type"::Order, "Posted Quality Order Header"."Order No.")then begin
                    if Vendor.Get(PurchaseOrder."Buy-from Vendor No.")then;
                end
                else
                begin
                    if PurchaseOrder.Get(PurchaseOrder."document type"::Order, "Posted Quality Order Header"."Source Document No.")then if Vendor.Get(PurchaseOrder."Buy-from Vendor No.")then;
                end;
                PurchRcptLine.Reset;
                PurchRcptLine.SetRange(PurchRcptLine."No.", "Posted Quality Order Header"."Item No.");
                PurchRcptLine.SetRange(PurchRcptLine."Posted Quality Order No.", "Posted Quality Order Header"."No.");
