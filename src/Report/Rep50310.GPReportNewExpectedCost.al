Report 50310 "GP Report New Expected Cost"
{
    // AlleZav1.02/060815 -- Ravik -- Added Sales Credit Memo Entries in the report
    DefaultLayout = RDLC;
    RDLCLayout = './Layouts/GP Report New Expected Cost.rdl';
        ...
        dataitem("Sales Invoice Header"; "Sales Invoice Header")
        {
            ...
            dataitem("Sales Invoice Line"; "Sales Invoice Line")
            {
                ...
                begin
                    ...
                    Clear(ExpectedCost);
                    Clear(CostCalcMgt);
                    RowID2:=RowID1; ////ItemTrackingMgt.ComposeRowID(DATABASE::"Sales Invoice Line",0,"Document No.",'',0,"Line No.")
                    ValueEntryRelation.SetCurrentkey("Source RowId");
                    ValueEntryRelation.SetRange("Source RowId", RowID2);
                    if ValueEntryRelation.Find('-')then begin
                        ...
                        ItemLedgEntry2.SetRange("Lot No.", LotNo);
                        if ItemLedgEntry2.FindSet then begin
                            ProdOrderLine.SetRange(Status, ProdOrderLine.Status::Released);
                            ProdOrderLine.SetRange("Prod. Order No.", ItemLedgEntry2."Document No.");
                            //ProdOrderLine.SETRANGE("Planning Level Code",0);
                            ProdOrderLine.SetFilter("Item No.", '%1', ItemLedgEntry2."Item No.");
                            if ProdOrderLine.Find('-')then begin
                                CostCalcMgt.CalcShareOfTotalCapCost(ProdOrderLine, ShareOfTotalCapCost);
                                CostCalcMgt.CalcProdOrderLineExpCost(ProdOrderLine, ShareOfTotalCapCost, ExpCost[1], ExpCost[2], ExpCost[3], ExpCost[4], ExpCost[5]);
                                ExpectedCost:=ExpCost[1] + ExpCost[2] + ExpCost[3] + ExpCost[4] + ExpCost[5];
                                ExpectedCost:=ProdOrderLine.Quantity;
                            end;
            ...
            dataitem("Sales Cr.Memo Line"; "Sales Cr.Memo Line")
            {
                ...
                begin
                    ...
                    if ILE.FindFirst then begin
                        ...
                        if "Line Amount" <> 0 then ProfitPer1:=((("Line Amount" - COGSCR) / "Line Amount") * 100);
                    end;
                end;
                trigger OnPreDataItem()
                begin
    ...
    ExpCost: array[10]of Decimal;
    ShareOfTotalCapCost: Decimal;
    procedure RowID1(): Text[250]var
        ItemTrackingMgt: Codeunit "Item Tracking Management";
    begin
        exit(ItemTrackingMgt.ComposeRowID(Database::"Sales Invoice Line", 0, "Sales Invoice Line"."Document No.", '', 0, "Sales Invoice Line"."Line No."));
    end;
}
