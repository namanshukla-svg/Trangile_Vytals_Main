Report 50009 "NEFT Document"
{
    // ALLE 2.15....NEFT/RTGS
    DefaultLayout = RDLC;
    RDLCLayout = './Layouts/NEFT Document.rdlc';
    ApplicationArea = all;
    ...
    dataset
    {
        ...
        {
            DataItemTableView = sorting("Document No.", "Posting Date", Amount)where("Transfer Type"=filter(NEFT), "Vendor Bank Account"=filter(<>''), "NEFT / RTGS Code"=filter(<>''));
            ...
            column(Document_No______Document_No__;'Document No.: ' + "Document No.")
            {
            }
            column(BankAccount_Name; BankAccount.Name)
            {
            }
            column(BankAccount_Address______BankAccount__Address_2_; BankAccount.Address + '  ' + BankAccount."Address 2")
            {
            }
            column(BankAccount_City_______BankAccount__Post_Code_; BankAccount.City + ' - ' + BankAccount."Post Code")
            {
            }
            ...
            column(G_L_Entry__G_L_Entry___NEFT___RTGS_Code_; "G/L Entry"."NEFT / RTGS Code")
            {
            }
            ...
            column(Sub__________National_Electronic_Fund_Transfer__NEFT__ApplicationCaption; Sub__________National_Electronic_Fund_Transfer__NEFT__ApplicationCaptionLbl)
            {
            }
            column(Kindly_transfer_fund_through_NEFT_to_the_beneficiary_as_detail_given_hereunder_Caption; Kindly_transfer_fund_through_NEFT_to_the_beneficiary_as_detail_given_hereunder_CaptionLbl)
            {
            }
            ...
            begin
                ...
                BankAccLedgerEntry.SetCurrentkey("Document No.", "Posting Date");
                BankAccLedgerEntry.SetRange("Document No.", "Document No.");
                if BankAccLedgerEntry.FindFirst then if BankAccount.Get(BankAccLedgerEntry."Bank Account No.")then;
            end;
    ...
    Sub__________National_Electronic_Fund_Transfer__NEFT__ApplicationCaptionLbl: label 'Sub       :  National Electronic Fund Transfer (NEFT) Application';
    Kindly_transfer_fund_through_NEFT_to_the_beneficiary_as_detail_given_hereunder_CaptionLbl: label 'Kindly transfer fund through NEFT to the beneficiary as detail given hereunder.';
