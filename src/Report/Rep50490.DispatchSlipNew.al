Report 50490 "Dispatch Slip New"
{
    DefaultLayout = RDLC;
    RDLCLayout = './Layouts/Dispatch Slip New.rdl';
    ApplicationArea = all;
        ...
        dataitem("Sales Header"; "Sales Header")
        {
            ...
            dataitem("Sales Line"; "Sales Line")
            {
                ...
                dataitem("Sales Schedule Buffer"; "SSD Sales Schedule Buffer")
                {
                    ...
                    column(ReportForNavId_1977;1977)
                    {
                    }
                ...
                begin
                    LotNo:='';
                    if "Sales Header"."Order/Scd. No." <> '' then begin
                        SalesShipmentHeaderRec.Reset();
                        SalesShipmentHeaderRec.SetRange("Order/Scd. No.", "Sales Header"."Order/Scd. No.");
                        if SalesShipmentHeaderRec.FindSet()then repeat SalesShipmentLineRec.Reset();
                                SalesShipmentLineRec.SetRange("Document No.", SalesShipmentHeaderRec."No.");
                                SalesShipmentLineRec.SetRange("No.", "Sales Line"."No.");
                                SalesShipmentLineRec.SetRange("Location Code", "Sales Line"."Location Code");
                                SalesShipmentLineRec.SetRange("Despatch Slip No.", "Sales Line"."Document No.");
                                SalesShipmentLineRec.SetRange("Despatch Slip Line No.", "Sales Line"."Line No.");
                                if SalesShipmentLineRec.FindSet()then repeat ItemLedgerEntryRec.Reset();
                                        ItemLedgerEntryRec.SetRange("Document Type", ItemLedgerEntryRec."Document Type"::"Sales Shipment");
                                        ItemLedgerEntryRec.SetRange("Document No.", SalesShipmentLineRec."Document No.");
                                        ItemLedgerEntryRec.SetRange("Document Line No.", SalesShipmentLineRec."Line No.");
                                        ItemLedgerEntryRec.SetRange("Item No.", SalesShipmentLineRec."No.");
                                        ItemLedgerEntryRec.SetRange("Location Code", SalesShipmentLineRec."Location Code");
                                        if ItemLedgerEntryRec.FindSet()then repeat if LotNo = '' then begin
                                                    LotNo:=ItemLedgerEntryRec."Lot No.";
                                                    QTY:=Format(Abs(ItemLedgerEntryRec.Quantity));
                                                end
                                                else
                                                begin
                                                    LotNo+='#' + ItemLedgerEntryRec."Lot No.";
