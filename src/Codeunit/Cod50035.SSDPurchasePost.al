Codeunit 50035 "SSD Purchase Post"
{
    ...
    begin
        ...
        PurchSetup.Get();
        if PurchSetup."Post with Job Queue" then PurchPostViaJobQueue.EnqueuePurchDoc(PurchaseHeader)
        else
        begin
    ...
    begin
        ...
        if PurchaseLine.FindSet()then repeat PurchaseLine.TestField("SSD Receipt Remarks");
    ...
    begin
        ...
        if SSDPOTerms.FindSet()then repeat if not AttachmentExists(PurchaseHeader4, SSDPOTerms."SSD Attachment Type")then error(AttachmentNotExistErr, SSDPOTerms."SSD Attachment Type");
        ...
        if not SSDPOTerms.IsEmpty then begin
            ...
            if SSDPOCheckList.FindSet()then repeat SSDPOCheckList.TestField(Completed);
                until SSDPOCheckList.Next() = 0;
        end;
    end;
    local procedure AttachmentExists(PurchaseHeader3: Record "Purchase Header"; AttachmentType: Code[20]): Boolean var
    ...
    begin
        ...
        if DocumentAttachment.IsEmpty then exit(false)
        else
            exit(true);
    end;
