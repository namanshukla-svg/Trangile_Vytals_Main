codeunit 50140 "SSD Transfer Order Subscriber"
{
    ...
    begin
        ...
        if TransferHeader."Posting Date" <> WorkDate then begin
            ...
            if not Confirm(StrSubstNo(MsgTxt, TransferHeader."Posting Date", WorkDate), false) then
        ...
        if TransferLine.FINDSET() then
            repeat
                ...
                if (Location."Phy. Bin Required") and (Item."Phy. Bin Required") then begin
                    ...
                    if not SSDItemPhyBinDetails.FINDFIRST() then
                        ERROR('Please fill the Item Phy. Details in Line No. %1', TransferLine."Line No.")
                    else begin
    ...
    var
        MsgTxt: Text;
    begin
        if PassedServHeader."Posting Date" <> WorkDate then begin
            MsgTxt :=
               'Posting Date (%1) is not equal to Work Date (%2). Do you want to continue Posting?';

            if not Confirm(StrSubstNo(MsgTxt, PassedServHeader."Posting Date", WorkDate), false) then
                Error('Posting cancelled by user.');
        end;
    end;
